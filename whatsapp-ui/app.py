...
function setPairingCode(value){const raw=String(value||'').trim();const code=raw.replace(/[^a-z0-9]/gi,'');if(code.length>0&&code.length<=12){$('code').textContent=code.match(/.{1,4}/g).join(' ');}else{$('code').textContent='متاح عبر QR';}}
async function refresh(force=false){try{if(force)await api('/whatsapp/api/create',{method:'POST'});const s=await api('/whatsapp/api/status');const state=s.instance?.state||s.instance?.status||s.state||'unknown';setState(state);if(String(state).toLowerCase()==='open')return;const q=await api('/whatsapp/api/qr');const b=q.qrcode?.base64||q.base64;const img=$('qr');if(b){if(String(b).startsWith('data:image/')){img.src=b;}else if(window.QRCode){img.src=await QRCode.toDataURL(String(b),{width:520,margin:2,errorCorrectionLevel:'M'});}else{throw new Error('QR renderer unavailable');}img.hidden=false;$('qrEmpty').hidden=true;}if(q.code)setPairingCode(q.code);}catch(e){$('pill').textContent='غير متصل';$('pill').style.background='#8a3040';$('stateHint').textContent='تعذر الاتصال بالخدمة';$('qrEmpty').textContent='تعذر تحميل رمز QR';}}
